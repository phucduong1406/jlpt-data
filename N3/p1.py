...
def load_and_merge_exam(json_path):
    # TEMP: render từng json riêng để reset draw_qno
...
def merge_parts(cover1, body1, cover2, body2, output_path):
    writer = PdfWriter()
...
if __name__ == "__main__":
    ...
    for f in sorted(json_files, key=json_sort_key):
        m = re.match(rf"{TARGET_BASE}-(\d+)\.json$", f)
        if not m or m.group(1) not in TARGET_PARTS:
            continue

        render_exam(f)

    # ===== 2. Gom PDF theo cặp N*-1 / N*-2 =====
    pdf_map = {}

    for f in os.listdir("."):
        if not f.lower().endswith(".pdf"):
            continue
        if f in ("cover-3-1.pdf", "cover-3-2.pdf"):
            continue
        ...
        pdf_map.setdefault(base, {})[idx] = f

    # ===== 3. Merge: cover-3-1 → N*-1 → cover-3-2 → N*-2 =====
    for base, parts in pdf_map.items():
        ...
        merge_parts(
            cover1="cover-3-1.pdf",
            body1=parts["1"],
            cover2="cover-3-2.pdf",
            body2=parts["2"],
            output_path=output
        )
